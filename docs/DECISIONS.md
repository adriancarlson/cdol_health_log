...
66. The approved import CSV includes the expanded complaint defaults and Recess. Conversation Type defaults are Email, Phone, and In Person; `Multiple Methods` is not a stored catch-all option, and the import intentionally contains no `Other` records.
