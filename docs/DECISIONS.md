...
55. Medication removal types use `MED_REMOVAL_TYPE` in the shared option table. The initial import preserves the five former hard-coded transaction codes so existing audit records and reports remain compatible. System-owned administration/correction transaction types remain application-controlled. Treatment remains free text unless a later requirement explicitly converts it to a controlled dropdown.
