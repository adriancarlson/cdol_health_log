...
- Show exactly one blue-outlined bottle with a solid blue cap and red plus for the selected student
  ...
  Display it at 21 by 28 pixels, preserving proportions. The original 15-by-20
  outlined-cap variant is retained as an unused backup.
- Navigate in the same tab to
  `/admin/students/medication/administration.html?frn=~(studentfrn)`.
...
The query returns no row when there is no gap and one aggregate row otherwise.
...
Run `py docs/tests/test_missed_medication_count.py` (25 tests) and
...
`http://127.0.0.1:8769/tests/student_alert.html` for a 21-by-28-pixel preview alongside
the reference icon and 15-by-20 backup. The fixture renders the shipped anchor with a fictional FRN.
...
1. Install Health Log 26.8.7.18 and hard-refresh a student page using an authorized
