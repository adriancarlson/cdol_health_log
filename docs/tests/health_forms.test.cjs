        ...
        for (const [scenario, file] of ['medical_authorization.html', 'health_history.html', 'medical_authorization.html'].entries()) {
            ...
            if (renderedNames) {
                ...
                await page.evaluate(() => {
                    document.querySelectorAll('input[name^="["], select[name^="["], textarea[name^="["]').forEach(field => {
                        ...
                        field.name = sourceName.replace(/^\[students\.([^\]]+)\](.+)$/i, 'EF-001123-$1.$2').replace(/^\[01\](.+)$/i, 'UF-001123-$1');
            ...
            await page.evaluate(startEmpty => {
                ...
                window.studentData = { student_dcid: '123', record_exists: 1, today: '2026-09-23',
                    med_share_consent: '1', med_first_aid_consent: '1', hospital_consent: '1',
                ...
                if (startEmpty) window.medications = [];
            ...
            if (scenario === 2) {
                ...
                console.log('PASS empty prescriptions: metadata-only response, no warning, first medication creation, audit, malformed-response rejection');
                await page.close();
                continue;
            }
