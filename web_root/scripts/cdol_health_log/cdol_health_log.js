define(['angular', 'components/shared/index', '/scripts/cdol/services/formatService.js', '/scripts/cdol/services/psApiService.js', '/scripts/cdol/services/emailService.js'], function (angular) {
	var cdolHealthLogApp = angular.module('cdolHealthLogAppMod', ['powerSchoolModule', 'formatService', 'psApiModule', 'emailModule'])
	cdolHealthLogApp.controller('cdolHealthLogAppCtrl', function ($scope, $http, $attrs, $window, formatService, psApiService, emailService) {
		...
		//pull existing Staff Change Record and setting it to submitPayload if an HealthLogId was provided through URL Params
		$scope.getHealthLog = async HealthLogId => {
			loadingDialog()
			//copyFormatKeys
			// let getFomatKeys = await { ...$scope.formatKeys }

			if (HealthLogId) {
				const res = await psApiService.psApiCall(`U_CDOL_STAFF_CHANGES`, `GET`, getFomatKeys, HealthLogId)
